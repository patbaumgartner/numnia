...
| Slash command (`/`) | Purpose | Input | Output |
| --- | --- | --- | --- |
...
These slash commands are stored as prompts under `.github/prompts/` and are invokable in chat via `/<name>`.
...
│   ├── entity_model.md                    ← (produced via /entity-model)
│   └── adr/                               ← Architecture Decision Records
├── backend/                               ← Spring Boot 4.0, Java 25
│   ├── pom.xml
│   └── src/{main,test}/java/...
├── frontend/                              ← React 19 + Babylon.js 7 + Vite 6
│   ├── package.json
│   └── src/...
...
└── .github/
    ...
    ├── instructions/                      ← *.instructions.md (per applyTo)
    ├── prompts/                           ← AIUP slash commands
...
9. **When in doubt, do not guess** - the SRS/UCs are the truth; if a statement is missing, ask or propose an ADR draft.
...
- Review by a second person or via the `/review` prompt.
