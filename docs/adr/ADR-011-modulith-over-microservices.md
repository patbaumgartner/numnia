# ADR-011 - Modulithic Spring Boot Backend Instead of Microservices
...
Numnia consists of multiple bounded contexts (see ADR-007), but is operated by a 1-2 person part-time team and hosted on a single Swiss node (ADR-010). We must decide whether to deploy each context as its own service (microservices) or as one cohesive backend with strong internal module boundaries (modulith), and which tooling enforces those boundaries.

## Decision Drivers

- Operability with a small team, no 24/7 staff (NFR-OPS-001).
- Single-node Swiss hosting (NFR-OPS-003).
- Clear module boundaries from ADR-007, enforceable inside one process.
- Releases R1-R3 do not require independent scaling per bounded context.

## Considered Options

- **Option A** - Modulithic Spring Boot 4.0.6 backend with **Spring Modulith 2.0.6** to define, verify, test and document bounded-context modules (ADR-007) inside one deployable.
...
We choose **Option A**. The backend is one Spring Boot 4.0.6 application composed of the bounded-context modules from ADR-007, plus a separately scalable WebSocket node sharing the same codebase via configuration.
...
Milestone / RC pre-releases (e.g., Spring Modulith 2.1.0-RC1, which targets Spring Boot 4.1 M4) are explicitly **not** adopted, consistent with the stable-only policy from ADR-002.
...
- ADR-010 (docker-compose orchestration)
