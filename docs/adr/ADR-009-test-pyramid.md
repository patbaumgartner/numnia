# ADR-009 - Test Pyramid: JUnit/AssertJ/Mockito + Testcontainers, Cucumber for BDD, Playwright for E2E
...
Numnia mandates Test First, TDD for business logic, BDD for acceptance criteria, and CI-enforced quality gates (NFR-ENG-002..004). We need a coherent test pyramid that the small operations team can run locally and in CI in reasonable time, and that supports the version pins from ADR-002.
...
- Stack pins from ADR-002 (Java 25, Spring Boot 4.0.6, Spring 7.0.7).
